...
import Types from "./flavor_database.js";
...
function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        let flavorName = product.flavor_id.map(flavor_id => 
            Types.find(flavor => flavor.id === flavor_id)?.name
        );
...
function displayFlavorFilter() {
    ...
    allButton.addEventListener("click", () => {
        ...
        flavorFilterSection.querySelectorAll("button.active").forEach(btn => btn.classList.remove("active"));
        allButton.classList.add("active");
        filterProducts("all"); // Show all products
    });

    // Create buttons for each flavor
    Types.forEach(flavor => {
        let button = document.createElement("button");
        button.setAttribute("id", flavor.id);
        button.textContent = flavor.name;

        // Event listener for each flavor button
        button.addEventListener("click", () => {
            button.classList.toggle("active"); // Toggle active state

            // Get all active flavor buttons
            
            flavorFilterSection.querySelectorAll("button.active").forEach(btn => btn.classList.remove("active"));
            filterProducts(flavor.id); 
            button.classList.add("active");
         
            filterProducts(flavor.id); // Pass selected flavor IDs to filter
            
        });

        flavorFilterSection.appendChild(button); // Add flavor button to the section
    });
}



function filterProducts(selectedFlavors) {
    let productsToDisplay = products_list; // Start with all products

    // If selectedFlavors is not "all", filter by selected flavor IDs
    if (selectedFlavors !== "all") {
        productsToDisplay = products_list.filter(product => 
            product.flavor_id.includes(selectedFlavors) // Check if the product includes the selected flavor
        );
    }
